...
public class PhysicsTester : MonoBehaviour
{
    ...
    {
        // Calculates the dot product between the forward vector and the projected forward vector on the XZ plane
        Vector3 forward = transform.forward;
        Vector3 projectedForward = Vector3.ProjectOnPlane(forward, Vector3.up).normalized;

        // Vector3.Dot to get the cosine of the angle between the forward vector and the projected forward vector
        ...
        // Determine the sign based on whether player is looking up or down
